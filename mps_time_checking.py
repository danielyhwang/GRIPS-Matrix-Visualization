# Generate the full corrected Python script with OOM-safe heatmap handling

safe_gui_benchmark_script = """
...
def process_file(filepath):
    ...
    try:
        start = time.time()
        if A_sparse.shape[0] * A_sparse.shape[1] > 10_000_000:
            print(f"⚠️ Skipping heatmap for large matrix: {A_sparse.shape}")
            record["row_scaled_heatmap_time"] = None
        else:
            A = A_sparse.toarray()
...
def main():
    ...
    if not files:
        print("❌ No MPS files found in mps/")
        return
...
# Save to file
script_path = "safe_gui_benchmark_runner.py"
with open(script_path, "w", encoding="utf-8") as f:
    f.write(safe_gui_benchmark_script)
